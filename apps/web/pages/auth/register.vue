...
// Modern registration page with Clerk authentication
...
const handleEmailSubmit = async () => {
  ...
  try {
    ...
    // Use Clerk's email sign-up flow
    await clerk.value.client.signUp.create({
    ...
    await router.push('/auth/verify-request')
            ...
            error.value = errorMessage || t('auth.registration_failed')
          } finally {
    loading.value = false
  }
}
...
const registerWithGoogle = async () => {
  try {
    ...
    await clerk.value.client.signUp.authenticateWithRedirect({
    ...
    error.value = err.errors?.[0]?.message || 'Google Registration fehlgeschlagen'
  } finally {
    loading.value = false
  }
}

// Social registration with Apple
const registerWithApple = async () => {
  try {
    ...
    error.value = err.errors?.[0]?.message || 'Apple Registration fehlgeschlagen'
  } finally {
    loading.value = false
  }
}
